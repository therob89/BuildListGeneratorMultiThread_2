...
class Config(BpmDataType):
        ...
        try:
            with open(self.list_file, 'w') as list_file:
                ...
                for _cnf in self.cnf_list:
                    ...
                    self.release_note_objects_by_server[server].add("bpm_cf_is" + rel_string)
                for _cache in cache_ds:
                    server, _cnf, rel_string = self.get_server_from_property_and_return_element(_cache)
                    self.config_by_server[server].add("bpm_is_cache_" + _cnf)
                    #self.release_note_objects_by_server[server].add("bpm_is_cache" + rel_string)
                    self.cache_list.append(("bpm_is_cache_" + _cnf))
                for server in self.server_list:
    ...
    def get_output(self):
        return self.output_config, self.cnf_res, self.cache_list
